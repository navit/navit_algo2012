...
//------------------------------------------------------------------------------------
ofVec2f vectorField::getForceFromPos(float xpos, float ypos){//mapping from screen to VF to get position
	
	ofVec2f frc;
	frc.set(0,0);
	
	// convert xpos and ypos into pcts = 
	float xPct = xpos / (float)externalWidth;//grabing the x position & dividing it by the total amount of the screen=pct 
	float yPct = ypos / (float)externalHeight;
	
	// if we are less then 0 or greater then 1 in x or y, return no force.
	if ((xPct < 0 || xPct > 1) || (yPct < 0 || yPct > 1)){ //if we are in the corners return an empty
		return frc;	
	} 
	
    // where are we in the array
    int fieldPosX = (int)(xPct * fieldWidth);//number of columns
    ...
    for (int i = startx; i < endx; i++){
        for (int j = starty; j < endy; j++){
			...
			if (distance < fieldRadius){
                ...
                float unit_px = ( fieldPosX - i) / distance;
                float unit_py = ( fieldPosY - j) / distance;
                field[pos].x -= unit_px * strongness;//difference in the code
                field[pos].y -= unit_py * strongness;
            }
        }
    }
