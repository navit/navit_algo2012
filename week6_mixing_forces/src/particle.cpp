...
void particle::draw(){
    ...
    float angle =atan2(diff.x,diff.y) + PI;
    ...
    for (int i = 0 ; i < trail.size() ; i++){
        ofPushMatrix();
        ofTranslate(diff.x, diff.y-sine*10000);
        float alpha = ofMap(i, angle, trail.size(), 0.0, 0.8);
        lineMesh.addColor( ofFloatColor(255.0/255.0, 0/225.0, 127.0/255.0, alpha));
        lineMesh.addVertex(trail[i]);
        ofPopMatrix();
        
        ofPushMatrix();
        ofTranslate(ofGetWidth(), ofGetHeight()-sine*100);
        float alpha1 = ofMap(i, angle, trail.size(), 0.0, 0.4);
        lineMesh.addColor( ofFloatColor(200.0/255.0, 240/225.0, 160.0/255.0, alpha));
        lineMesh.addVertex(trail[i]);
        ofPopMatrix();
    }
